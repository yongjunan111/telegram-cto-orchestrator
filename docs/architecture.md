...
When room or task acceptance criteria are defined, `handoff complete` accepts `--task-criterion-cover <index>:<evidence>` and `--room-criterion-cover <index>:<evidence>` to map each criterion to its evidence. The coverage is stored in `resolution.acceptance_coverage` as `{task: [...], room: [...]}`. `handoff review` displays per-criterion coverage status in an Acceptance Coverage section alongside the Validation Coverage section. `handoff approve` enforces a hard gate: all defined acceptance criteria (both task-level and room-level) must have explicit coverage before approval is allowed. The execution brief surfaces coverage instructions when criteria are defined.
...
**Compiled wiki layer**: The repo contains a compiled operating knowledge layer under `.orchestrator/wiki/`. This is not authoritative state. Its role is to preserve current state, design rationale, deferred work, recurring patterns, and lessons across session compression and runtime/model changes. The authority boundary remains strict — code, room/handoff YAML state, and git history are the source of truth.
